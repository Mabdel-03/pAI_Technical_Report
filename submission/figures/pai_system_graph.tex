...
\begin{tikzpicture}[
  ...
  label/.style={font=\sffamily\bfseries\Large,anchor=west}
]

\node[label] at (-5.12,3.74) {pAI research-to-manuscript graph};
\node[font=\sffamily\normalsize,anchor=west,text=black!70] at (-5.12,3.43)
...
\node[gate] (merge) at (-1.65,-1.00) {track\\merge};
...
\node[stage,fill=purple!8] (write) at (-4.15,-2.28) {Resource prep\\and paper contract};
...
\draw[arr] (e2) -- (e3);
\draw[arr] (e3) -- (e4);
\draw[soft] (t2.south) -- node[right,font=\scriptsize,align=center] {predictions} (e2.north);
\draw[soft] (e3.north) -- node[left,font=\scriptsize,align=center] {counterexamples\\and metrics} (t3.south);
...
% Legend
\node[stage,minimum width=0.62in,minimum height=0.23in,fill=white,font=\sffamily\footnotesize] (la) at (-4.58,-3.58) {agent};
\node[gate,minimum width=0.38in,minimum height=0.20in] (lg) [right=0.22in of la] {};
\node[human,minimum width=0.48in,minimum height=0.22in,font=\sffamily\footnotesize] (lh) [right=0.28in of lg] {human};
\draw[arr] ($(lh.east)+(0.22,0)$) -- +(0.55,0);
\draw[soft] ($(lh.east)+(1.03,0)$) -- +(0.55,0);
\node[anchor=west,font=\sffamily\footnotesize,text=black!70] at ($(lh.east)+(1.75,0)$)
